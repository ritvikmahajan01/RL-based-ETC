...
for i in range(episodes):
    ...
    print(i, " Score: %.2f" % score, " RL IET = %.3f" % te_avg_rl, " Classic IET = %.3f" % te_avg_classical, " RL/Classical = %.3f" % rl_by_classical)
    write_csv([i, score, x0, t_series[-1], te_avg_rl, te_avg_classical, rl_by_classical])


plt.figure(1)
plt.plot(range(episodes),tavg_rl,'g:',label='RL')
plt.plot(range(episodes),tavg_classical,'b-',label='Classical')
plt.ylabel('IET')
plt.xlabel('Episode')
plt.legend(loc='best')
name = "plots/IET"
...
plt.figure(2)
plt.plot(range(episodes), score_history)
plt.ylabel('IET(RL) / IET(Classical) - 1')
plt.xlabel('Episode')
