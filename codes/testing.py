...
for i in range(episodes):
    theta = (i/episodes)*4*math.pi
    x_init = np.array([math.cos(theta), math.sin(theta)])
    x0 = np.copy(x_init)
    done = False
    agent.noise.reset()
    ...
    t_lyap, x_lyap, tevent_, iet_series_lyap, u_lyap = F.lyap_etc_episode(sys1, x_init, t)
    ...
    print(i, "RL Dis_Sum = %.3f" % iet_dis_sum_rl, " Lyap Dis_sum = %.3f" % iet_dis_sum_lyap, "RL Avg IET = %.3f" % avg_iet_rl, " Lyap Avg IET = %.3f" % avg_iet_lyap)

diff_dis_sum = np.mean(rl_dis_sum) - np.mean(lyap_dis_sum)
diff_avg = np.mean(rl_vs_lyap_avg)
print("Difference between mean of:\nDiscounted sum of IETs = %.3f" % diff_dis_sum, "\nAverage IET = %.3f" % diff_avg)
